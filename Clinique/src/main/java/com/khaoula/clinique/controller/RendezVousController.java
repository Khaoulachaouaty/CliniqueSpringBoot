...
import com.khaoula.clinique.services.RendezVousService;
...
public class RendezVousController {
    ...
    public ResponseEntity<RendezVousResponse> updateStatus(
            @PathVariable Long id,
            @Valid @RequestBody ConfirmRendezVousRequest request) {
        logger.info("🔄 Changement statut RDV: {} -> {}", id, request.getStatut());
        return ResponseEntity.ok(rendezVousService.updateStatus(id, request.getStatut()));
    }
    ...
    // 🔥 NOUVEAU : Endpoint pour créneaux occupés
    ...
    // 🔥 SUPPRIMÉ : Tous les endpoints /disponibilites/*
}