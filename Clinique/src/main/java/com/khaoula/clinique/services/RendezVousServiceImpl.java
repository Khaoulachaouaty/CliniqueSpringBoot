...
public class RendezVousServiceImpl implements RendezVousService {
    ...
    @Override
    public RendezVousResponse updateStatus(Long rendezVousId, String status) {
        logger.info("🔄 Mise à jour statut RDV {} → {}", rendezVousId, status);
        
        RendezVous rdv = rendezVousRepository.findById(rendezVousId)
                .orElseThrow(() -> new RuntimeException("RDV non trouvé"));
        
        List<String> statutsValides = Arrays.asList("EN_ATTENTE", "CONFIRME", "ANNULE", "TERMINE");
        ...
        // 🔥 Notifications selon l'action du MÉDECIN vers le PATIENT
        if ("CONFIRME".equals(status)) {
            notificationService.notifierConfirmationRdv(saved);
        } else if ("ANNULE".equals(status)) {
            notificationService.notifierAnnulationRdvParMedecin(saved, "Annulé par le médecin");
        }
        
        logger.info("✅ Statut mis à jour: RDV {} est maintenant {}", rendezVousId, status);
